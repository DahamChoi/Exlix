    ...
    public void OnPointerClick(PointerEventData eventData) {
        ...
        if(results.Count > 0) {
            if (results[0].gameObject.transform.GetComponent<CardObject>()) {
                UIState.GetInstance()._UIStateHandler.UpdateSelectedCard(results[0].gameObject.transform.GetComponent<CardObject>().CardData);
                CardDTO card = UIState.GetInstance()._UIStateHandler.GetSelectedCard();
                CardExplain.text = card.Explanation;
                CardCost.text = card.Cost.ToString();
                CardTitle.text = card.Title;
